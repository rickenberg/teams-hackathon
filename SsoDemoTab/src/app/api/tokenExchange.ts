...
export const tokenExchange = (options: any): express.Router => {
  ...
  // Define a method used to exchange the identity token to an access token
  ...
  router.get(
    ...
    async (
      ...
      let scopes = [""];
      if (process.env.SSODEMO_APP_SCOPES !== undefined) {
          scopes = process.env.SSODEMO_APP_SCOPES.split(",");
      }
      try {
        const accessToken = await exchangeForToken(
          user.tid,
